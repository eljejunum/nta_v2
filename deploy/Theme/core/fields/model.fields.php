...
/**
 * The PHP code for setup model page custom fields.
 *
 * @package WordPress
 */
...
function model_create_meta_box() {

	global $model_postmetas;
	if ( function_exists('add_meta_box') && isset($model_postmetas) && count($model_postmetas) > 0 ) {  
		add_meta_box( 'page_metabox', 'Page Options', 'model_new_meta_box', 'models', 'side', 'high' );  
	}

}  

function model_new_meta_box() {
	global $post, $model_postmetas;
	...
	foreach ( $model_postmetas as $postmeta ) {
		...
		if ($meta_type == 'checkbox') {
			...
			if(!empty($postmeta['items']))
			{
				...
				{
					$model_style = get_post_meta($post->ID, $meta_id);
				
					if(isset($model_style[0]) && $item_key == $model_style[0])
					{
						$css_string = 'selected';
					}
					else
...
function model_save_postdata( $post_id ) {
	...
	if ( isset($_POST['post_type']) && 'model' == $_POST['post_type'] ) {
		if ( !current_user_can( 'edit_model', $post_id ) )
			return $post_id;
		} else {
	...
	foreach ( $model_postmetas as $postmeta ) {
		...
		if ($_POST[$postmeta['id']]) {
			model_update_custom_meta($post_id, $_POST[$postmeta['id']], $postmeta['id']);
		}
...
function model_update_custom_meta($postID, $newvalue, $field_name) {
...
add_action('admin_menu', 'model_create_meta_box'); 
add_action('save_post', 'model_save_postdata'); 
